...
if __name__ == '__main__':
    ...
    def get_noize(N=44100, color='white', state=None, rate=44100, second=2):
        wav = []
        count = 0
        for noize in noise_generator(N=N, color=color, state=state):
            noize /= 3
...
#            print(noize)
    ...
    for color in _noise_generators.keys():
        print(color)
        wav = Sampler.Sampler().Sampling(get_noize(N=hz, color=color, state=None, rate=hz, second=5))
        p.Play(wav, rate=hz)
